...
function init_top_menu() {
	...
	for (var i in menu_mids) {
		...
		if (menu_data[mid]) {
			...
			if (has_sub_menu) {
				a_menu[ni] = "<a id='mt"+mid+"' href='"+click_link+"' onclick='load("+menu_stru[mid][0]+");return false'"; //���ص�һ���Ӳ˵�������
			} else {
...
function load(mid) {
	...
	if (show_shortcut && menu_shortcut) {
		var shortcut_tmp = "<table class='leftmenu_2'><tr><td class='head'>��ݷ�ʽ</td></tr>";
...
function make_navi(string) {
	var navi_split = " �� ";
	var title_array = ('������̨,'+string).split(',');
...
function get_online() {
	...
	online_error_timer = setTimeout("get_online_error()", 15000); //15s��ʱ
}

// ������Ϣ�������:
function get_online_do(out) {
	//document.title = "�Ѳ��˹���  "+new Date().toTimeString().substring(0,8)+" "+((new Date().getTime() - online_last_send_time) / 1000);
	document.title = ori_doc_title;
	clearTimeout(online_error_timer); //����ֹͣ��������Ӧ����
...
/*function show_online_list(aOnline) {
	if (typeof(aOnline) == typeof(window)) {
		...
		string += "<tr><td class='head'><div style='float:left;'>�����û�</div><div style='float:right;'><a href='javascript:void(0);' onclick=\"load_url('m/sys/online_all.php')\" title='��ʾ���������û�'>����>></a></div><div class='clear'></div></td></tr>";
		for (var n in aOnline) {
			string += "<tr><td class='item' onmouseover='mi(this)' onmouseout='mo(this)'>";
			if (n > 0) {
				string += "<a href='#"+n+":"+aOnline[n]["name"]+"' onclick=\"load_box(1, 'src', 'm/sys/admin.php?op=viewweb&name="+aOnline[n]["name"]+"'); return false;\" title='"+(aOnline[n]["isowner"] != 1 ? "�鿴�û�����" : "�鿴�ҵ�����")+"'>";
			}
			...
			if (aOnline[n]["isowner"] != 1) {
				string += "&nbsp;<a href='javascript:void(0)' onclick=\"load_box(1, 'src', 'm/sys/talk.php?to="+n+"');\" class='talk' title='���������Ϣ'>[��̸]</a>";
			}
			string + "</td></tr>";
		}
		string += "</table>";
		...
		var string = "<div class='online_tips'>û�������û�����</div>";
	}
...
function show_online_message(aMess) {
	if (typeof(aMess) == typeof(window)) {
		...
		for (var mid in aMess) {
			if (!mid) continue;
			string += "<table width='100%' id='online_mess_" + mid + "' style='border:1px solid #E1E1E1'><tr><td width='75%' style='padding:3px 6px 3px 6px' style='color:#000000'>" + aMess[mid]["time"] + "&nbsp;<font color='#FF5F11'><b>" + aMess[mid]["realname"] + "</b></font> ˵��</td><td width='25%' align='right' style='padding-right:6px'><a href='#' onclick=\"online_talk('" + aMess[mid]["fromname"] + "','" + aMess[mid]["realname"] + "'," + mid + ");return false\" class='talk_op'>[�ظ�]</a>&nbsp;<a href='#' onclick='online_talk_close(" + mid + ");return false' title='���Ѿ��Ķ�,��Ҫ����ʾ' class='talk_op'>[�ر�]</a></td></tr><tr><td colspan='2' style='padding:6px' style='color:#000000'>";
			if (aMess[mid]["link"] != '') {
...
function get_online_list() {
	...
	if (obj.innerHTML == "") {
		obj.innerHTML = "<div class='online_tips'>���ڶ�ȡ�����û�...</div>";
	}
	oAjax = new ajax();
...
//	if (typeof(aOnline) == typeof(window)) {
//		var string = "<table width='100%' class='leftmenu_online'>";
//		string += "<tr><td class='head'>�����û�</td></tr>";
//		for (var n in aOnline) {
//			string += "<tr><td class='item' onmouseover='mi(this)' onmouseout='mo(this)'>";
//			string += "<a href='#' onclick=\"load_url('sys_admin_view.php?name="+n+"','�����û�,�û�����'); return false;\" title='"+(aOnline[n]["isowner"] != 1 ? "�鿴�û�����" : "�鿴�ҵ�����")+"'>"+aOnline[n]["realname"]+"</a>";
//			if (aOnline[n]["isowner"] != 1) {
//				string += "&nbsp;<a href='#' onclick=\"online_talk('"+n+"','"+aOnline[n]["realname"]+"');return false;\" class='talk' title='���������Ϣ'>[��̸]</a>";
//			}
//			string + "</td></tr>";
//		}
//		string += "</table>";
...
function online_talk(username, realname, messid) {
	...
	string += "<tr><td colspan='2' style='padding:4px 3px 2px 6px'>�� <font color='red'><b>" + realname + "</b></font> ������Ϣ:&nbsp;&nbsp;&nbsp;<a href='#' onclick='show_face(this);return false;'>[����]</a></td></tr>";
	string += "<tr><td width='20%' align='center'>���ݣ�</td><td width='80%'><textarea id='online_talk_content' style='width:90%;height:80px;' class='input'></textarea></td></tr>";
	string += "<tr><td colspan='2' height='60' align='center'><input type='button' class='submit' onclick=\"online_talk_submit('" + username + "',"+messid+")\" value='������Ϣ'>&nbsp;&nbsp;<input type='button' class='submit' onclick='online_talk_hide()' value='ȡ��'></td></tr>";
	string += "</table>";
...
function get_online_messages_do(oAjax) {
	...
	if (typeof(aMess) == typeof(window)) {
		...
		for (var mid in aMess) {
			if (!mid) continue;
			string += "<table width='100%' id='online_mess_" + mid + "' style='border:1px solid #E1E1E1'><tr><td width='75%' style='padding:3px 6px 3px 6px' style='color:#000000'>" + aMess[mid]["time"] + "&nbsp;<font color='#FF5F11'><b>" + aMess[mid]["realname"] + "</b></font> ˵��</td><td width='25%' align='right' style='padding-right:6px'><a href='#' onclick=\"online_talk('" + aMess[mid]["fromname"] + "','" + aMess[mid]["realname"] + "'," + mid + ");return false\" class='talk_op'>[�ظ�]</a>&nbsp;<a href='#' onclick='online_talk_close(" + mid + ");return false' title='���Ѿ��Ķ�,��Ҫ����ʾ' class='talk_op'>[�ر�]</a></td></tr><tr><td colspan='2' style='padding:6px' style='color:#000000'><a href='"+aMess[mid]["link"]+"' target='main'><b>" + aMess[mid]["content"] + "</b></a></td></tr></table>";
		}
		var obj = byid("online_message");
		if (string && typeof(obj) == typeof(window)) {
			obj.innerHTML += string;
...
function set_body_height() {
	var all = get_size()[3];
	var main_bar_height = all - byid("top_border").offsetHeight - byid("logo_bar").offsetHeight - byid("menu_bar").offsetHeight - byid("bottom_border").offsetHeight - 6; // 6 ������paddingֵ

	var frame_base_height = main_bar_height - 0; //iframe�Ļ�׼�߶�(�պ����ҳ��ĸ߶�)
...
function load_src(isshow, src) {
	if (isshow) {
		...
		var ow = Math.max(400, width - 300); //�����Ŀ���
		var oh = Math.max(300, wh - 60); //�����ĸ߶�
